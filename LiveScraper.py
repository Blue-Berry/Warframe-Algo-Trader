...
try:
    while config.getConfigStatus("runningLiveScraper"):
        ...
        t3 = time.time()
        if interstingItems_counter % 10 == 0 or interstingItems_counter == 1:
            buySellOverlap = getBuySellOverlap()
            interestingItems = list(buySellOverlap.index)
        ...
        read_inventory_time = 0
        get_live_orders_time = 0
        compare_orders_time = 0
        total_time = 0
        ...
        for item in interestingItems:
            ...
            logging.debug(f"Item number {item_count} out of {len(interestingItems)}")
            # Read inventory from database
            t7 = time.time()
            con = sqlite3.connect('inventory.db')
            ...
            t12 = time.time()
            compare_orders_time += t12 - t11
        # Calculate total time
        total_time = read_inventory_time + get_live_orders_time + compare_orders_time
        # Print percentage of time spent in each area
        logging.debug(f"Percentage of time spent in each area:")
        logging.debug(f"  Read inventory: {read_inventory_time / total_time * 100:.2f}%")
        logging.debug(f"  Get live orders: {get_live_orders_time / total_time * 100:.2f}%")
        logging.debug(f"  Compare orders: {compare_orders_time / total_time * 100:.2f}%")
        ...
        # Iterate over the InterestingItems list and remove any items that don't have a buy order
    for item in interestingItems.copy():
        if item not in myBuyOrdersDF['item'].tolist():
            interestingItems.remove(item)
